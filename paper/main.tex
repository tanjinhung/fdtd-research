...
The design process began with a baseline implementation where the 3D FDTD update equations, as defined in \eqref{eq:full_fdtd_vacuum}, were ported to HLS using a standard software-based approach \cite{schneider-2010}.
In this initial stage, the field components were represented as multidimensional arrays in global memory.
To establish a performance baseline, basic optimizations were applied by placing the \texttt{\#pragma HLS PIPELINE II=1} directive on the inner-most loops of the update functions.
