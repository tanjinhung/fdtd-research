...
static void update_H_tail() {
#pragma HLS INLINE off
  update_HZ();
}

static void update_EX_tail() {
#pragma HLS INLINE off
...
static void update_E_tail() {
#pragma HLS INLINE off
  update_EX_tail();
  update_EY_tail();
}

static void check_dipole(const int z) {
#pragma HLS INLINE off
  for (int y = 1; y < NY_1; ++y) {
    for (int x = 1; x < NX_1; ++x) {
#pragma HLS PIPELINE II = 1
...
static void stage_read(const float *__restrict__ hx_gmem,
                       const float *__restrict__ hy_gmem,
                       const float *__restrict__ hz_gmem,
                       const float *__restrict__ ex_gmem,
                       const float *__restrict__ ey_gmem,
                       const float *__restrict__ ez_gmem, const int z) {
  if (z < NZ_1) {
...
static void stage_compute(const int z) {
  if (z < NZ_1) {
    update_H_crit();
    update_E_crit();
    check_dipole(z);
...
static void stage_write(float *__restrict__ hx_gmem,
                        float *__restrict__ hy_gmem,
                        float *__restrict__ hz_gmem,
                        float *__restrict__ ex_gmem,
                        float *__restrict__ ey_gmem,
                        float *__restrict__ ez_gmem, const int z) {
  if (z < NZ_1) {
...
#pragma HLS PIPELINE II = 1
  ...
  for (int z = 0; z < NZ_0; ++z) {
    stage_read(hx_gmem, hy_gmem, hz_gmem, ex_gmem, ey_gmem, ez_gmem, z);
    stage_compute(z);
    stage_write(hx_gmem, hy_gmem, hz_gmem, ex_gmem, ey_gmem, ez_gmem, z);
  }
}
